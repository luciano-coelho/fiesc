...
flowchart TD
    ...
    B --> B2[Google Colab]
    B --> B3[CLI]
    B --> B4[Testes Automatizados]
    
    B --> C[NLP]
    C --> D[Gerador SQL]
    D --> E[Banco de Dados]
    E --> F[Resultados Formatados]
    F --> G[Interface]
...
sequenceDiagram
    ...
    participant N as NLP & Regex
    participant S as Gerador SQL
    participant DB as Banco de Dados
    participant R as Resposta

    U->>I: Digita pergunta
    I->>N: Envia texto
    N->>S: Retorna intenção e parâmetros
    S->>DB: Executa SQL
    DB->>S: Retorna dados
    S->>R: Formata resposta
    R->>I: Retorna resposta final
    I->>U: Exibe resposta
...
erDiagram
    ...
    TECNICOS ||--o{ ORDEM_TECNICO : participa
    EQUIPAMENTOS ||--o{ ORDENS_MANUTENCAO : possui
    ORDENS_MANUTENCAO ||--o{ ORDEM_TECNICO : atribui
